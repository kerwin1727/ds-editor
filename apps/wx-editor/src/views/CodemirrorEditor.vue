...
const isMdBootstrapped = ref(false)
const mdLoadProgress = ref(0)
const mdDisplayProgress = computed(() => Math.round(mdLoadProgress.value))

let mdProgressTimer: ReturnType<typeof window.setInterval> | null = null
let mdProgressFallbackTimer: ReturnType<typeof window.setTimeout> | null = null
...
const MD_REFRESH_DRAFT_KEY = `wx-md-refresh-draft`
const MD_REFRESH_DRAFT_TIME_KEY = `wx-md-refresh-draft-updated-at`
const mdRefreshDraft = useStorage<string>(MD_REFRESH_DRAFT_KEY, ``)
const mdRefreshDraftUpdatedAt = useStorage<number>(MD_REFRESH_DRAFT_TIME_KEY, 0)

function hasActivePost() {
  return Boolean(store.currentPostId && store._currentPost)
}

function persistMdRefreshDraft(content: string) {
  mdRefreshDraft.value = content
  mdRefreshDraftUpdatedAt.value = Date.now()
}

function getInitialEditorContent() {
  if (hasActivePost()) {
    return store._currentPost?.content || ``
  }
  return mdRefreshDraft.value || ``
}
...
let controlsTimer: ReturnType<typeof window.setTimeout> | undefined
...
// 修改滚动处理函数
function stopMdProgressSimulation() {
  if (mdProgressTimer !== null) {
    window.clearInterval(mdProgressTimer)
    mdProgressTimer = null
  }
}

function clearMdProgressFallbackTimer() {
  if (mdProgressFallbackTimer !== null) {
    window.clearTimeout(mdProgressFallbackTimer)
    mdProgressFallbackTimer = null
  }
}

function startMdProgressSimulation() {
  isMdBootstrapped.value = false
  mdLoadProgress.value = 6
  stopMdProgressSimulation()

  mdProgressTimer = window.setInterval(() => {
    if (mdLoadProgress.value >= 90) {
      return
    }

    const remaining = 90 - mdLoadProgress.value
    const step = Math.max(0.7, remaining * 0.11)
    mdLoadProgress.value = Math.min(90, mdLoadProgress.value + step)
  }, 110)
}

async function finalizeMdBootLoading() {
  if (isMdBootstrapped.value) {
    return
  }

  clearMdProgressFallbackTimer()
  stopMdProgressSimulation()
  mdLoadProgress.value = 100

  await new Promise<void>((resolve) => {
    window.setTimeout(resolve, 170)
  })

  isMdBootstrapped.value = true
}
...
const timeout = ref<ReturnType<typeof window.setTimeout>>()
const changeTimer = ref<ReturnType<typeof window.setTimeout>>()
...
function initEditor() {
  ...
  // 设置初始内容
  const initialContent = getInitialEditorContent()
  if (!editorDom.value) {
    editorDom.value = initialContent
  }
...
// 修改组件挂载逻辑
onMounted(() => {
  startMdProgressSimulation()
  clearMdProgressFallbackTimer()
  mdProgressFallbackTimer = window.setTimeout(() => {
    void finalizeMdBootLoading()
  }, 2600)
...
const editorPreviewContent = ref<HTMLDivElement | null>(null)
const isResizingPane = ref(false)
const editorPanePercent = useStorage<number>(`wx-editor-pane-percent`, 50)

const MIN_EDITOR_PANE_PERCENT = 25
const MAX_EDITOR_PANE_PERCENT = 75

const safeEditorPanePercent = computed(() => {
  return Math.min(MAX_EDITOR_PANE_PERCENT, Math.max(MIN_EDITOR_PANE_PERCENT, editorPanePercent.value))
})

const previewPanePercent = computed(() => 100 - safeEditorPanePercent.value)

const throttledRefreshEditor = useThrottleFn(() => {
  editor.value?.refresh()
}, 16)

function clampEditorPanePercent(percent: number) {
  return Math.min(MAX_EDITOR_PANE_PERCENT, Math.max(MIN_EDITOR_PANE_PERCENT, percent))
}

function applyPaneWidthFromClientX(clientX: number) {
  if (!editorPreviewContent.value)
    return

  const rect = editorPreviewContent.value.getBoundingClientRect()
  if (rect.width <= 0)
    return

  const relativePercent = ((clientX - rect.left) / rect.width) * 100
  const nextEditorPercent = store.isEditOnLeft ? relativePercent : 100 - relativePercent
  editorPanePercent.value = clampEditorPanePercent(nextEditorPercent)
}

function handlePaneResizeMove(event: PointerEvent) {
  if (!isResizingPane.value)
    return
  applyPaneWidthFromClientX(event.clientX)
}

function stopEditorPreviewResize() {
  if (!isResizingPane.value)
    return

  isResizingPane.value = false
  document.body.classList.remove(`editor-preview-resizing`)
  window.removeEventListener(`pointermove`, handlePaneResizeMove)
  window.removeEventListener(`pointerup`, stopEditorPreviewResize)
  window.removeEventListener(`pointercancel`, stopEditorPreviewResize)
  nextTick(() => editor.value?.refresh())
}

function startEditorPreviewResize(event: PointerEvent) {
  if (event.button !== 0)
    return
  ...
  document.body.classList.add(`editor-preview-resizing`)
  window.addEventListener(`pointermove`, handlePaneResizeMove)
  window.addEventListener(`pointerup`, stopEditorPreviewResize)
  window.addEventListener(`pointercancel`, stopEditorPreviewResize)
}

watch(safeEditorPanePercent, (value) => {
  if (value !== editorPanePercent.value) {
    editorPanePercent.value = value
    return
  }

  nextTick(() => {
    throttledRefreshEditor()
  })
}, { immediate: true })
...
// 处理平台审核文章导入事件
function handleInsertPlatformArticle(event: CustomEvent) {
  const { content, mode, articleId, title } = event.detail || {}
  if (!editor.value || !content) {
    return
  }
  ...
  if (articleId) {
    ;(window as any).currentPlatformArticleId = articleId
  }

  if (title && store._currentPost) {
    const index = store.posts.findIndex(p => store._currentPost && p.id === store._currentPost.id)
    if (index !== -1) {
      store.renamePost(index, title)
    }
  }

  store.editorRefresh()
  toast.success(`已导入审核文章到编辑器`)
}
...
onMounted(() => {
  ...
  document.addEventListener(`insert-platform-article`, handleInsertPlatformArticle as EventListener)
})
...
onUnmounted(() => {
  ...
  document.removeEventListener(`insert-platform-article`, handleInsertPlatformArticle as EventListener)
...
<template>
  <div ref="container" class="container flex flex-col">
    ...
    <main class="container-main flex flex-1 flex-col">
      <div v-if="!isMdBootstrapped" class="md-editor-loading">
        <div class="md-loading-card">
          <div class="md-loading-line w-72" />
          <div class="md-loading-line w-96" />
          <div class="md-loading-line w-80" />
          <div class="md-loading-line w-60" />
          <div class="md-loading-progress" role="progressbar" :aria-valuenow="mdDisplayProgress" aria-valuemin="0" aria-valuemax="100">
            <div class="md-loading-progress-fill" :style="{ width: `${mdDisplayProgress}%` }">
              <span class="md-loading-rocket" aria-hidden="true">🚀</span>
            </div>
          </div>
        </div>
        <div class="md-loading-text">
          Markdown editor loading... {{ mdDisplayProgress }}%
        </div>
      </div>
      <div class="container-main-section border-radius-10 relative flex flex-1 overflow-hidden border-1">
        ...
        <div class="editor-preview-container flex flex-1 flex-col overflow-hidden">
          ...
          <div ref="editorPreviewContent" class="editor-preview-content flex flex-1 overflow-hidden" :class="{ 'is-resizing': isResizingPane }">
            <div
              ref="codeMirrorWrapper" class="codeMirror-wrapper relative" :class="{
                'border-r': store.isEditOnLeft,
                'border-l': !store.isEditOnLeft,
                'is-transitioning': isTransitioning,
              }" :style="{
                flex: `0 0 ${safeEditorPanePercent}%`,
                order: store.isEditOnLeft ? 0 : 2,
              }"
            >
              <ContextMenu modal>
                <ContextMenuTrigger>
                  <textarea
                    id="editor" type="textarea" placeholder="Your markdown text here."
            ...
            <div
              class="editor-preview-divider" :class="{ 'is-active': isResizingPane }" :style="{ order: 1 }" role="separator"
              aria-orientation="vertical" @pointerdown="startEditorPreviewResize"
            />

            <div
              id="preview" ref="preview" class="preview-wrapper flex-1 p-5" :class="{
                'is-transitioning': isTransitioning,
              }" :style="{
                flex: `0 0 ${previewPanePercent}%`,
                order: store.isEditOnLeft ? 2 : 0,
              }"
            >
...
.md-loading-line {
  ...
  animation: md-loading-shimmer 1.2s linear infinite;
}

.md-loading-line:first-child {
  margin-top: 0;
}

.md-loading-line.w-96 {
  width: min(100%, 24rem);
}

.md-loading-line.w-80 {
  width: min(100%, 20rem);
}

.md-loading-line.w-72 {
  width: min(100%, 18rem);
}

.md-loading-line.w-60 {
  width: min(100%, 15rem);
}
...
.md-loading-progress-fill {
  ...
  animation: md-loading-progress-flow 1.4s linear infinite;
}
...
.md-loading-rocket {
  ...
  animation: md-loading-rocket-float 0.78s ease-in-out infinite alternate, md-loading-rocket-boost 1.3s ease-in-out infinite;
}

.md-loading-rocket::after {
  content: '';
  position: absolute;
  right: 90%;
  top: 50%;
  ...
  animation: md-loading-trail-flicker 0.34s ease-in-out infinite alternate;
}
...
.editor-preview-divider {
  ...
  &::before {
    ...
    height: 100%;
    border-radius: 999px;
    background-color: rgb(148 163 184 / 45%);
...
:global(body.editor-preview-resizing) {
  user-select: none;
  cursor: col-resize;
}

@keyframes md-loading-shimmer {
  from {
...
@keyframes md-loading-rocket-float {
  from {
...
@keyframes md-loading-rocket-boost {
  from {
    scale: 1;
  }
  to {
...
@keyframes md-loading-trail-flicker {
  from {
    opacity: 0.65;
    width: 8px;
  }
...
@keyframes md-loading-progress-flow {
  from {
