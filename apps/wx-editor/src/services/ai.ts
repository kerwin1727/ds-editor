...
function isGenericHttpStatusText(statusText: string) {
  return /^(?:Bad Request|Unauthorized|Forbidden|Not Found|Too Many Requests|Internal Server Error|Service Unavailable)$/i.test(statusText)
}

function extractMessageFromErrorPayload(payload: any): string | null {
  if (!payload)
    return null
  ...
  if (payload.error && typeof payload.error === 'object') {
    const nestedErrorMessage = payload.error.message || payload.error.msg || payload.error.detail
    if (typeof nestedErrorMessage === 'string' && nestedErrorMessage.trim()) {
      return nestedErrorMessage.trim()
    }
  }

  const message = payload.message || payload.msg || payload.detail || payload.error_description
...
async function getResponseErrorDetail(response: Response): Promise<string | undefined> {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      const message = extractMessageFromErrorPayload(data)
      if (message)
        return message
...
function formatErrorMessage(status: number, providerId: string, statusText?: string) {
  ...
  const baseMessage = adapter.formatErrorMessage(status, statusText)
  const detail = statusText?.trim()

  if (!detail || isGenericHttpStatusText(detail) || baseMessage.includes(detail)) {
    return baseMessage
  }
...
function isCodexLikeModel(model: string) {
...
function normalizeModelForRequest(model: string) {
  const trimmed = model.trim()
  if (!trimmed) {
    return trimmed
  }

  if (/^gpt-[\w.-]+$/i.test(trimmed) || /^o\d[\w.-]*$/i.test(trimmed) || trimmed.toLowerCase().includes('codex')) {
...
function buildRequestMessages(
  prompt: AIStreamOptions['prompt'],
  presetWords: string[],
) {
  return [
    ...presetWords.map(word => ({ role: `system` as const, content: word })),
...
function buildChatCompletionRequestBody(options: {
  model: string
  ...
  const { model, messages, temperature, maxLength, stream, deepThinking } = options
  const modelId = normalizeModelForRequest(model)
  ...
  const codexLike = isCodexLikeModel(modelId)

  if (!codexLike) {
    requestBody.temperature = temperature
    requestBody.max_tokens = maxLength
    ...
    // 对 GPT-5/Codex 等模型优先使用 max_completion_tokens
    requestBody.max_completion_tokens = maxLength
  }

  return requestBody
}

function buildMinimalFallbackBody(requestBody: any) {
  const minimal = {
  ...
  const after = JSON.stringify(minimal)
...
async function postChatCompletionWithFallback(options: {
  apiUrl: string
  headers: Record<string, string>
  body: any
  signal?: AbortSignal
}) {
  const { apiUrl, headers, body, signal } = options
  ...
  const fallbackBody = buildMinimalFallbackBody(body)
  if (!fallbackBody) {
    return response
  }
...
export async function streamAIContent({
  ...
  try {
    ...
    const requestBody = buildChatCompletionRequestBody({
      ...
      messages: buildRequestMessages(processedPrompt, aiStore.currentPresetWords),
      temperature: deepThinking ? Math.min(aiStore.currentTemperature, 0.3) : aiStore.currentTemperature,
      maxLength: aiStore.currentMaxLength,
      stream: true,
      deepThinking,
    })
    ...
    if (!response.ok) {
      ...
      const errorMessage = formatErrorMessage(response.status, aiStore.currentProviderId, errorDetail || response.statusText)
      throw new Error(errorMessage)
    }

    // 部分中转服务会忽略 stream=true 并返回普通 JSON，这里做兼容处理。
    const contentType = response.headers.get('content-type') || ''
    ...
    if (!isEventStream) {
      ...
      try {
        ...
        content = adapter.parseContent(data)
          || extractMessageFromErrorPayload(data)
          || JSON.stringify(data)
      }
      catch {
