...
type FrontMatterParsed = {
  attributes?: Record<string, any>
  body?: string
}

function parseScalarYamlValue(value: string): any {
...
function parseSimpleFrontMatterYaml(yamlText: string): Record<string, any> {
  ...
  yamlText.split(/\r?\n/).forEach((line) => {
    ...
    if (listMatch && currentArrayKey) {
      if (!Array.isArray(attributes[currentArrayKey])) {
        attributes[currentArrayKey] = []
      }
      attributes[currentArrayKey].push(parseScalarYamlValue(listMatch[1]))
      return
    }
    ...
    attributes[key] = parseScalarYamlValue(rawValue)
    currentArrayKey = null
  })
...
function extractFrontMatter(markdownText: string): FrontMatterParsed {
  const normalized = markdownText.replace(/\r\n?/g, `\n`)
  if (!normalized.startsWith(`---\n`) && !normalized.startsWith(`= yaml =\n`)) {
    return {
      attributes: {},
      body: markdownText,
    }
  ...
  const startMarker = lines[0].trim()
  const endLineIndex = lines.findIndex((line, index) => {
    if (index === 0) {
      return false
    }
    const trimmed = line.trim()
  ...
  const yamlText = lines.slice(1, endLineIndex).join(`\n`)
  const body = lines.slice(endLineIndex + 1).join(`\n`)

  return {
    attributes: parseSimpleFrontMatterYaml(yamlText),
    body,
  }
}
...
function parseFrontMatterAndContent(markdownText: string): ParseResult {
  try {
    const parsed = extractFrontMatter(markdownText)
    const yamlData = parsed.attributes ?? {}
    const markdownContent = parsed.body ?? markdownText
  ...
  const renderer: RendererObject = {
    ...
    listitem(item: Tokens.ListItem): string {
      ...
      if (item.task) {
        ...
        return styledContent(`listitem`, `${checkbox} ${content}`, `li`)
      }
    ...
    link({ href, title, text, tokens }: Tokens.Link): string {
      ...
      // 仅在特定条件下跳过链接渲染，避免影响引用式链接
      if (href === text && !title) {
        // 如果链接文本和URL相同且没有标题，可能是自引用，可以选择跳过
        // 但在大多数情况下，我们仍应显示为链接
