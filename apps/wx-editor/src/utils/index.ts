...
type JuiceFn = (html: string, options?: {
  inlinePseudoElements?: boolean
  preserveImportant?: boolean
}) => string

function getGlobalJuice(): JuiceFn | null {
  const maybeJuice = (globalThis as { juice?: unknown }).juice
  return typeof maybeJuice === `function` ? maybeJuice as JuiceFn : null
}
...
export function mergeCss(html: string): string {
  ...
  try {
    ...
    console.warn(`mergeCss failed, fallback to raw html`, error)
    return html
  }
}
